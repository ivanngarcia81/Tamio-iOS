...
struct MockMiembrosRepository: MiembrosRepository {
    ...
    static var activosCount: Int { almacen.filter { !$0.estado.esBaja }.count }

    func lista(filtro: FiltroMiembro) async throws -> [Aportante] {
        try? await Task.sleep(nanoseconds: 120_000_000)
        switch filtro {
        case .activos: return Self.almacen.filter { !$0.estado.esBaja }
        case .bajas: return Self.almacen.filter { $0.estado.esBaja }
    ...
    private static var todos: [Aportante] {
        ...
        return [
            ...
            // Javier tiene un traslado EN CURSO: sigue activo hasta que se
            // cierre. El expediente vivirá en `traslados_salida`.
            mk("2", "Javier Medina Cruz", .activo, L.t("diezmo", "tithe"), "2016", "81 8899 1020", "jmedina@outlook.com", 26_200_00, "MECJ870920K44", .semanal, 26),
            ...
            mk("10", "Tomás Robles Leal", .baja("2025-11-02", "retiro"), L.t("baja", "removed"), "2013", "81 2323 4545", "tomas.robles@correo.mx", 6_800_00, "ROLT800606QR9", .ocasional, 200),
            mk("11", "Verónica Ibarra Sosa", .baja("2026-02-18", "traslado"), L.t("baja", "removed"), "2015", "81 6767 8989", "veronica.ibarra@correo.mx", 3_900_00, "IASV860404ST0", .ocasional, 300),
        ]
    }
}
